...
{
 public:
  MinimalistPrinter(int procId, MPI_Comm comm) : mProcId(procId), mNumFails(0), m_comm(comm) {}
 ...
 protected:
  ...
  {
    printf("%s on proc %d in %s:%d\n%s\n", test_result_string(test_part_result).c_str(), mProcId,
  ...
  void OnTestIterationEnd(const ::testing::UnitTest& unit_test, int /*iteration*/)
  {
    ...
    if (mProcId == 0) {
      ...
      if (mNumFails > 0) {
        ...
        for (const std::string& testName : failedTestNames) print_failed("--gtest_filter=" + testName);
      }
      if (should_print_time()) {
  ...
  std::vector<std::string> get_failed_test_names(const ::testing::UnitTest& unit_test)
  {
    ...
    for (int i = 0; i < unit_test.total_test_case_count(); i++) {
      ...
      if (testCase->Failed()) append_failed_test_names_for_case(testCase, failedTestNames);
    }
...
class MinimalistPrinterOnlyOutputOnFailure : public MinimalistPrinter
{
 public:
  ...
  {
    const int numTotalFailures = get_num_failures_across_procs(test_info);
    if (numTotalFailures > 0) printf("%s", mOutputCapturer->get_and_reset_captured_output().c_str());
