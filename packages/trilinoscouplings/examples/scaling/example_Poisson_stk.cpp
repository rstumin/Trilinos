...
/** \brief  User-defined material tensor.
    ...
    \warning Symmetric and positive definite tensor is required for every
   (x,y,z).
*/
template <typename Scalar>
void materialTensor(Scalar material[][3], const Scalar &x, const Scalar &y,
                    const Scalar &z);

/** \brief  Computes gradient of the exact solution. Requires user-defined exact
   solution.

    \param  gradExact  [out]   gradient of the exact solution evaluated at
   (x,y,z) \param  x          [in]    x-coordinate of the evaluation point
    \param  y          [in]    y-coordinate of the evaluation point
    \param  z          [in]    z-coordinate of the evaluation point
 */
template <typename Scalar>
void exactSolutionGrad(Scalar gradExact[3], const Scalar &x, const Scalar &y,
                       const Scalar &z);

/** \brief Computes source term: f = -div(A.grad u).  Requires user-defined
   exact solution and material tensor.

    \param  x          [in]    x-coordinate of the evaluation point
    \param  y          [in]    y-coordinate of the evaluation point
    \param  z          [in]    z-coordinate of the evaluation point

    \return Source term corresponding to the user-defined exact solution
   evaluated at (x,y,z)
 */
template <typename Scalar>
const Scalar sourceTerm(Scalar &x, Scalar &y, Scalar &z);

/** \brief Computation of the material tensor at array of points in physical
   space.

    \param worksetMaterialValues      [out]     Rank-2, 3 or 4 array with
   dimensions (C,P), (C,P,D) or (C,P,D,D) with the values of the material tensor
...
/** \brief Computation of the source term at array of points in physical space.
    ...
    \param sourceTermValues           [out]     Rank-2 (C,P) array with the
   values of the source term \param evaluationPoints           [in]      Rank-3
   (C,P,D) array with the evaluation points in physical frame
*/
template <class ArrayOut, class ArrayIn>
void evaluateSourceTerm(ArrayOut &sourceTermValues,
                        const ArrayIn &evaluationPoints);

/** \brief Computation of the exact solution at array of points in physical
   space.

    \param exactSolutionValues        [out]     Rank-2 (C,P) array with the
   values of the exact solution \param evaluationPoints           [in] Rank-3
   (C,P,D) array with the evaluation points in physical frame
*/
template <class ArrayOut, class ArrayIn>
...
/** \brief Computation of the gradient of the exact solution at array of points
   in physical space.

    \param exactSolutionGradValues    [out]     Rank-3 (C,P,D) array with the
   values of the gradient of the exact solution \param evaluationPoints [in]
   Rank-3 (C,P,D) array with the evaluation points in physical frame
*/
template <class ArrayOut, class ArrayIn>
...
using Xpetra_Matrix = Xpetra::Matrix<SC, LO, GO, Tpetra_Vector::node_type>;
int TestMultiLevelPreconditioner(
    char ProblemType[], Teuchos::ParameterList &MLList,
    Teuchos::RCP<Tpetra_CrsMatrix> &A,
    Teuchos::RCP<Tpetra_CrsMatrix> &MassMatrix,
    Teuchos::RCP<Xpetra_Matrix> &Minv, Teuchos::RCP<Tpetra_Vector> &xexact,
    Teuchos::RCP<Tpetra_MultiVector> &b, Teuchos::RCP<Tpetra_MultiVector> &uh,
    Teuchos::RCP<Tpetra_MultiVector> &coords, double &TotalErrorResidual,
...
void mesh_read_write(const std::string &type,
                     ...
                     stk::io::StkMeshIoBroker &broker, int db_integer_size,
  ...
  size_t input_index =
      broker.add_mesh_database(absoluteFileName, type, stk::io::READ_MESH);
...
/**********************************************************************************/
/**********************************************************************************/
template <class crs_matrix_type, class multivector_type1,
          class multivector_type2, class solution_type>
void Apply_Dirichlet_BCs(std::vector<int> &BCNodes, crs_matrix_type &A,
                         multivector_type1 &x, multivector_type2 &b,
                         solution_type &solution_values) {
  ...
  if (b.getMap()->getComm()->getRank() == 0)
    std::cout << "Apply in Dirichlet BCs to " << BCNodes.size() << " nodes"
              << std::endl;
  ...
  for (int i = 0; i < N; i++) {
    ...
    typename crs_matrix_type::nonconst_local_inds_host_view_type cols(
        "cols", numEntriesInRow);
    typename crs_matrix_type::nonconst_values_host_view_type vals(
        "vals", numEntriesInRow);
...
int main_(int argc, char *argv[]) {
  ...
  clp.setOption("mesh", &optMeshFile,
                "Exodus hexahedra, tetrahedral, quadrilateral or triangle mesh "
                "file with nodeset define for boundary");
  std::string optXmlFile = "";
  clp.setOption("xml", &optXmlFile, "xml file containing ML solver options");
  bool optPrintLocalStats = false;
  clp.setOption("localstats", "nolocalstats", &optPrintLocalStats,
                "print per-process statistics");
  // If matrixFilename is nonempty, dump the matrix to that file
  // in MatrixMarket format.
  std::string matrixFilename;
  clp.setOption("matrixFilename", &matrixFilename,
                "If nonempty, dump the "
                "generated matrix to that file in MatrixMarket format.");

  // If rhsFilename is nonempty, dump the rhs to that file
  // in MatrixMarket format.
  std::string rhsFilename;
  clp.setOption("rhsFilename", &rhsFilename,
                "If nonempty, dump the "
  ...
  clp.setOption("initialGuessFilename", &initialGuessFilename,
                "If nonempty, dump the "
  ...
  clp.setOption("coordsFilename", &coordsFilename,
                "If nonempty, dump the "
                "generated coordinates to that file in MatrixMarket format.");

  bool buildMassMatrix = false;
  clp.setOption("buildMassMat", "dontBuildMassMat", &buildMassMatrix,
  ...
  if (MyPID == 0) {
    std::cout << "============================================================="
              ...
              << "|                                                            "
                 "                 |\n"
              << "|  Intrepid2's website: "
                 "http://trilinos.sandia.gov/packages/intrepid2         |\n"
  ...
  if (xmlSolverInFileName.length()) {
    if (MyPID == 0)
      std::cout << "\nReading parameter list from the XML file \""
                << xmlSolverInFileName << "\" ...\n"
                << std::endl;
    Teuchos::updateParametersFromXmlFile(xmlSolverInFileName,
                                         Teuchos::ptr(&inputSolverList));
  } else if (MyPID == 0)
  ...
  /**********************************************************************************/
  RCP<TimeMonitor> tm =
      rcp(new TimeMonitor(*TimeMonitor::getNewTimer("1) Read Mesh")));
  ...
  broker.property_add(Ioss::Property(
      "COMPOSE_RESULTS", false)); // Note!  true results in an error in Seacas
  ...
  mesh_read_write(type, working_directory, filename, broker, db_integer_size,
                  hb_type);
  ...
  int numLocalNodes =
      stk::mesh::count_entities(bulkData, NODE_RANK, locallyOwnedSelector);

  stk::mesh::for_each_entity_run(
      bulkData, NODE_RANK, locallyOwnedSelector,
  ...
  int numLocalElems =
      stk::mesh::count_entities(bulkData, ELEMENT_RANK, locallyOwnedSelector);

  if (optPrintLocalStats) {
    for (int i = 0; i < numRanks; ++i) {
      if (MyPID == i) {
        std::cout << "(" << MyPID
                  << ")    Number of local Elements: " << numLocalElems
                  << std::endl
                  << "(" << MyPID
                  << ")    Number of local Nodes   : " << numLocalNodes
                  << std::endl
                  << std::endl;
      }
      Comm->barrier();
  ...
  Teuchos::reduceAll(*Comm, Teuchos::REDUCE_SUM, (GO)numLocalNodes,
                     Teuchos::outArg(numGlobalNodes));
  Teuchos::reduceAll(*Comm, Teuchos::REDUCE_SUM, (GO)numLocalElems,
                     Teuchos::outArg(numGlobalElements));
  if (MyPID == 0) {
    std::cout << "       Number of global Nodes   : " << numGlobalNodes
              << std::endl;
    std::cout << "       Number of global Elements: " << numGlobalElements
              << std::endl;
  }
  tm = Teuchos::null;
  ...
  RCP<Tpetra_Map> globalMapG = Teuchos::rcp(new Tpetra_Map(
      (Tpetra::global_size_t)numGlobalNodes, ownedGIDs(), (GO)0, Comm));
  RCP<Tpetra_Map> ownedPlusSharedMapG =
      Teuchos::rcp(new Tpetra_Map((Tpetra::global_size_t)numGlobalNodes,
                                  ownedPlusSharedGIDs(), (GO)0, Comm));
  Kokkos::DualView<size_t *> nnzPerRowUpperBound("nnzbound",
                                                 ownedPlusSharedGIDs.size());
  nnzPerRowUpperBound.template modify<
      typename Kokkos::DualView<size_t *>::host_mirror_space>();
  auto nnzPerRowUpperBound_h = nnzPerRowUpperBound.view_host();

  // Count the local elements and get node index upper bound
  stk::mesh::for_each_entity_run(
      bulkData, ELEMENT_RANK, locallyOwnedSelector,
      [&](const stk::mesh::BulkData &mesh, stk::mesh::Entity elem) {
        stk::mesh::ConnectedEntities nodes =
            mesh.get_connected_entities(elem, NODE_RANK);

        // NOTE: This will substantially overcount the NNZ needed.  You should
        // use hash table to be smarter
  ...
  // Build the Graph
  RCP<Tpetra_FECrsGraph> StiffGraph = rcp(new Tpetra_FECrsGraph(
      globalMapG, ownedPlusSharedMapG, nnzPerRowUpperBound));
  ...
  stk::mesh::for_each_entity_run(
      ...
      [&](const stk::mesh::BulkData &mesh, stk::mesh::Entity elem) {
        ...
        if (!foundOwnedNode) {
          std::cout << "Warning, element " << mesh.identifier(elem) << " on P"
                    << mesh.parallel_rank()
                    << " doesn't have any locally-owned nodes." << std::endl;
        }
  ...
  tm = rcp(new TimeMonitor(
      *TimeMonitor::getNewTimer("3) Compute Coordinates and Stats")));
  ...
  for (const stk::mesh::Part *nodeSet : nodeSets) {
    ...
    if (bcNodes.size() == 0)
      ...
      std::copy(myBcNodes.begin(), myBcNodes.end(),
                std::back_inserter(bcNodes));
    }

    if (MyPID == 0)
      printf("Adding nodeset %s\n", nodeSet->name().c_str());
  }
  ...
  for (const stk::mesh::Part *elemBlock : elemBlocks) {
    ...
    // that all parts with elements (aka ELEMENT_RANK) have the same topology
    // type
    auto myCell =
        stk::mesh::get_cell_topology(metaData.get_topology(*elemBlock));
  ...
  if (MyPID == 0) {
    std::cout << "Cell Topology: " << cellType.getName() << " ("
              << cellType.getBaseName() << ")" << std::endl;
  }

  // MachineLearningStatistics_Hex3D<SC,LO,GO,NO>
  // MLStatistics(numGlobalElements);
  bool do_statistics =
      !strcmp(cellType.getName(), "Hexahedron_8") && (numRanks == 1);
  if (MyPID == 0)
  ...
  int numGlobalBCs = 0;
  Teuchos::reduceAll(*Comm, Teuchos::REDUCE_SUM, numLocalBCs,
                     Teuchos::outArg(numGlobalBCs));

  if (MyPID == 0) {
    std::cout << "       Number of Dirichlet Nodes: " << numGlobalBCs
              << std::endl;
    if (numGlobalBCs == 0) {
      std::cout << std::endl
  ...
  if (optPrintLocalStats) {
    for (int i = 0; i < numRanks; ++i) {
      if (MyPID == i) {
        std::cout << "(" << MyPID
                  << ")    Number of local b.c. nodes: " << bcNodes.size()
                  << std::endl;
      }
      Comm->barrier();
    }
  ...
  RCP<Intrepid2::Cubature<device_type>> cellCubature =
      cubFactory.create<device_type>(cellType, cubDegree);
  ...
  //  auto HGradBasis =
  //  Intrepid2::getBasis<Intrepid2::DerivedNodalBasisFamily<device_type>
  //  >(cellType, Intrepid2::FUNCTION_SPACE_HGRAD, order);
  auto HGradBasis = rcp(new Intrepid2::Basis_HGRAD_HEX_C1_FEM<device_type>());

  int numFieldsG = HGradBasis->getCardinality();

  Intrepid2ScalarView basisValues("HGBValues", numFieldsG, numCubPoints);
  Intrepid2ScalarView basisGrads("HGBGrads", numFieldsG, numCubPoints,
                                 spaceDim);
  ...
  RCP<Tpetra_MultiVector> nCoord =
      rcp(new Tpetra_MultiVector(globalMapG, spaceDim));
  auto nCoord_h = nCoord->get2dViewNonConst();
  // Loop over elements

  const stk::mesh::BucketVector &localElementBuckets =
      bulkData.get_buckets(ELEMENT_RANK, locallyOwnedSelector);
  for (const stk::mesh::Bucket *elemBucketPtr : localElementBuckets) {
    for (stk::mesh::Entity elem : *elemBucketPtr) {
      stk::mesh::ConnectedEntities nodes =
          bulkData.get_connected_entities(elem, NODE_RANK);
      for (unsigned inode = 0; inode < nodes.size(); ++inode) {
        double *coord = stk::mesh::field_data(*coords, nodes[inode]);
        LO lid = globalMapG->getLocalElement(
            (int)bulkData.identifier(nodes[inode]) - 1);
        if (lid != Teuchos::OrdinalTraits<LO>::invalid()) {
  ...
  /**********************************************************************************/
  if (do_statistics) {
    Intrepid2::ScalarView<int, memory_space> elemToNode(
        "elemToNode", numLocalElems, numNodesPerElem);
    Intrepid2::ScalarView<int, memory_space> elemToEdge(
        "elemToEdge", numLocalElems, numEdgesPerElem);
    Intrepid2::ScalarView<double, memory_space> nodeCoord(
        "nodeCoord", numLocalNodes, spaceDim);
    Intrepid2::ScalarView<double, memory_space> sigmaVal("sigmaVal",
                                                         numLocalElems);
    ...
    for (const stk::mesh::Bucket *elemBucketPtr : localElementBuckets) {
      for (stk::mesh::Entity elem : *elemBucketPtr) {
        ...
        for (unsigned iedge = 0; iedge < cellType.getEdgeCount(); iedge++) {
          ...
          int lid0 = globalMapG->getLocalElement(
              (int)bulkData.identifier(nodes[n0]) - 1);
          int lid1 = globalMapG->getLocalElement(
              (int)bulkData.identifier(nodes[n1]) - 1);
          if (lid0 != -1 && lid1 != -1) {
    ...
    Intrepid2::ScalarView<int, memory_space> edgeToNode("edgeToNode",
                                                        edge_vector.size(), 2);
  ...
  /**********************************************************************************/
  tm =
      rcp(new TimeMonitor(*TimeMonitor::getNewTimer("5) Matrix/RHS Assembly")));
  RCP<Tpetra_FECrsMatrix> StiffMatrix = rcp(new Tpetra_FECrsMatrix(StiffGraph));
  RCP<Tpetra_FECrsMatrix> MassMatrix;
  if (buildMassMatrix)
    MassMatrix = rcp(new Tpetra_FECrsMatrix(StiffGraph));

  // Define desired workset size and count how many worksets there are on this
  // processor's mesh block
  // int desiredWorksetSize = numElems;                   // change to desired
  // workset size!
  ////int desiredWorksetSize = 100;                      // change to desired
  ///workset size!
  ...
  // When numLocalElems is not divisible by desiredWorksetSize, increase workset
  // count by 1
  if (numWorksets * desiredWorksetSize < numLocalElems)
    numWorksets += 1;

  if (MyPID == 0) {
    std::cout << "       Desired workset size:                 "
              << desiredWorksetSize << std::endl;
  ...
  // Start assembly
  RCP<Tpetra_FEMultiVector> rhsVector =
      rcp(new Tpetra_FEMultiVector(globalMapG, StiffGraph->getImporter(), 1));
  if (buildMassMatrix)
    Tpetra::beginAssembly(*MassMatrix, *StiffMatrix, *rhsVector);
  else
  ...
  for (int workset = 0; workset < numWorksets; workset++) {
    ...
    // when numLocalElems is not divisible by desiredWorksetSize, the last
    // workset ends at numLocalElems
    worksetEnd = (worksetEnd <= numLocalElems) ? worksetEnd : numLocalElems;

    // allocate the array for the cell nodes
    worksetSize = worksetEnd - worksetBegin;
    Intrepid2ScalarView cellWorkset("cellWorkset", worksetSize, numNodesPerElem,
                                    spaceDim);
    ...
    // Containers for Jacobians, integration measure & cubature points in
    // workset cells
    Intrepid2ScalarView worksetJacobian("worksetJacobian", worksetSize,
                                        numCubPoints, spaceDim, spaceDim);
    Intrepid2ScalarView worksetJacobInv("worksetJacobInv", worksetSize,
                                        numCubPoints, spaceDim, spaceDim);
    Intrepid2ScalarView worksetJacobDet("worksetJacobDet", worksetSize,
                                        numCubPoints);
    Intrepid2ScalarView worksetCubWeights("worksetCubWeights", worksetSize,
                                          numCubPoints);
    Intrepid2ScalarView worksetCubPoints("worksetCubPoints", worksetSize,
                                         numCubPoints, cubDim);

    // Containers for basis values transformed to workset cells and them
    // multiplied by cubature weights
    Intrepid2ScalarView worksetBasisValues("worksetBasisValues", worksetSize,
                                           numFieldsG, numCubPoints);
    Intrepid2ScalarView worksetBasisValuesWeighted(
        "worksetBasisValuesWeighted", worksetSize, numFieldsG, numCubPoints);
    Intrepid2ScalarView worksetBasisGrads("worksetBasisGrads", worksetSize,
                                          numFieldsG, numCubPoints, spaceDim);
    Intrepid2ScalarView worksetBasisGradsWeighted("worksetBasisGradsWeighted",
                                                  worksetSize, numFieldsG,
                                                  numCubPoints, spaceDim);

    // Containers for diffusive & advective fluxes & non-conservative adv. term
    // and reactive terms
    Intrepid2ScalarView worksetDiffusiveFlux("worksetDiffusiveFlux",
                                             worksetSize, numFieldsG,
                                             numCubPoints, spaceDim);

    // Containers for material values and source term. Require user-defined
    // functions
    Intrepid2ScalarView worksetMaterialVals("worksetMaterialVals", worksetSize,
                                            numCubPoints, spaceDim, spaceDim);
    Intrepid2ScalarView worksetSourceTerm("worksetSourceTerm", worksetSize,
                                          numCubPoints);

    // Containers for workset contributions to the discretization matrix and the
    // right hand side
    Intrepid2ScalarView worksetStiffMatrix("worksetStiffMatrix", worksetSize,
                                           numFieldsG, numFieldsG);
    Intrepid2ScalarView worksetMassMatrix;
    if (buildMassMatrix)
      worksetMassMatrix = Intrepid2ScalarView("worksetMassMatrix", worksetSize,
                                              numFieldsG, numFieldsG);
    ...
    Intrepid2CTools::setJacobian(worksetJacobian, cubPoints, cellWorkset,
                                 cellType);
    ...
    Intrepid2CTools::mapToPhysicalFrame(worksetCubPoints, cubPoints,
    ...
    Intrepid2FSTools::HGRADtransformGRAD(worksetBasisGrads, worksetJacobInv,
                                         basisGrads);

    // Compute integration measure for workset cells: Det(DF)*w = J*w
    Intrepid2FSTools::computeCellMeasure(worksetCubWeights, worksetJacobDet,
                                         cubWeights);
    ...
    Intrepid2FSTools::tensorMultiplyDataField(
        worksetDiffusiveFlux, worksetMaterialVals, worksetBasisGrads);

    // Integrate to compute contribution to global stiffness matrix:
    // (DF^{-T}(grad u)*J*w)*(A*DF^{-T}(grad u))
    Intrepid2FSTools::integrate(worksetStiffMatrix, worksetBasisGradsWeighted,
    ...
    // Transform basis values to physical frame:                        clones
    // basis values (u)
    Intrepid2FSTools::HGRADtransformVALUE(worksetBasisValues, basisValues);
    ...
    // f.(u)*J*w
    Intrepid2FSTools::integrate(worksetRHS, worksetSourceTerm,
    ...
    if (buildMassMatrix)
      Intrepid2FSTools::integrate(worksetMassMatrix, worksetBasisValues,
                                  worksetBasisValuesWeighted);
    ...
    */
    ...
    for (const stk::mesh::Bucket *elemBucketPtr : localElementBuckets) {
      for (stk::mesh::Entity elem : *elemBucketPtr) {
        ...
        stk::mesh::ConnectedEntities worksetNodes =
            bulkData.get_connected_entities(elem, NODE_RANK);

        // "CELL EQUATION" loop for the workset cell: cellRow is relative to the
        // cell DoF numbering
        for (int cellRow = 0; cellRow < numFieldsG; cellRow++) {

          int globalRow = bulkData.identifier(worksetNodes[cellRow]) - 1;
          double sourceTermContribution =
              worksetRHS(worksetCellOrdinal, cellRow);
          rhsVector->sumIntoGlobalValue(globalRow, 0, sourceTermContribution);

          // "CELL VARIABLE" loop for the workset cell: cellCol is relative to
          // the cell DoF numbering
          for (int cellCol = 0; cellCol < numFieldsG; cellCol++) {
            ...
            val[0] = operatorMatrixContribution;
            StiffMatrix->sumIntoGlobalValues(globalRow, col(), val());

            if (buildMassMatrix) {
              double MassMatrixContribution =
                  worksetMassMatrix(worksetCellOrdinal, cellRow, cellCol);
              Teuchos::Array<SC> massval(1);
              massval[0] = MassMatrixContribution;
  ...
  if (buildMassMatrix)
    Tpetra::endAssembly(*MassMatrix, *StiffMatrix, *rhsVector);
  else
  ...
  */
  ...
  if(do_statistics){
    ...
    if(MyPID==0) std::cout<<"*** Problem Statistics
  ***"<<std::endl<<problemStatistics<<std::endl;
  }
  */
  tm = Teuchos::null;
  ...
  /**********************************************************************************/
  tm = rcp(new TimeMonitor(
      *TimeMonitor::getNewTimer("6) Matrix/RHS Dirichlet BCs")));
  RCP<Tpetra_MultiVector> lhsVector =
      rcp(new Tpetra_MultiVector(globalMapG, 1));

  std::vector<SC> solution_values;
  solution_values.reserve(bcNodes.size());
  std::vector<int> ownedBoundaryNodes;
  ownedBoundaryNodes.reserve(bcNodes.size());
  ...
  Apply_Dirichlet_BCs(ownedBoundaryNodes, *StiffMatrix, *lhsVector, *rhsVector,
                      solution_values);
  ...
  /**********************************************************************************/
  tm = rcp(new TimeMonitor(
      *TimeMonitor::getNewTimer("7) Analytic Solution Generation")));
  ...
  const stk::mesh::BucketVector &localNodeBuckets =
      bulkData.get_buckets(NODE_RANK, locallyOwnedSelector);
  ...
  Teuchos::RCP<Tpetra_CrsMatrix> Acrs =
      Teuchos::rcp_dynamic_cast<Tpetra_CrsMatrix>(StiffMatrix);
  Teuchos::RCP<Tpetra_MultiVector> rhsMV =
      Teuchos::rcp_dynamic_cast<Tpetra_MultiVector>(rhsVector);
  Teuchos::RCP<Tpetra_CrsMatrix> Mcrs;
  Teuchos::RCP<Xpetra::Matrix<SC, LO, GO, Tpetra_Vector::node_type>> Minv;

  if (buildMassMatrix) {
    Mcrs = Teuchos::rcp_dynamic_cast<Tpetra_CrsMatrix>(MassMatrix);
    auto jynk = Xpetra::toXpetra(Mcrs);
    Minv =
        MueLu::UtilitiesBase<SC, LO, GO, Tpetra_Vector::node_type>::SPAI(jynk);
  }
  tm = Teuchos::null;
...
//                                 MassMatrix,
#ifdef out
  TestMultiLevelPreconditioner(probType, MLList, Acrs, Mcrs, exactNodalVals,
                               rhsMV, lhsVector, nCoord, TotalErrorResidual,
                               TotalErrorExactSol);
#endif
  double tolVals[9] = {.64, .32, .16, .08, .04, .02, .01, .001, .0001};
  Teuchos::ParameterList &mueList = MLList.sublist("MueLu");
  for (int iii = 0; iii < 2; iii++) {
    if (iii == 0) {
      mueList.set("aggregation: drop scheme", "cut-drop");
      mueList.set("aggregation: strength-of-connection: measure",
                  "smoothed aggregation");
      mueList.set("aggregation: strength-of-connection: matrix", "MinvA");
    }
    if (iii == 1) {
      mueList.set("aggregation: drop scheme", "point-wise");
      mueList.set("aggregation: strength-of-connection: measure",
                  "smoothed aggregation");
      mueList.set("aggregation: strength-of-connection: matrix", "MinvA");
    }
    ...
    if (iii == 1) {
      ...
      mueList.set("aggregation: strength-of-connection: measure",
                  "signed ruge-stueben");
      mueList.set("aggregation: strength-of-connection: matrix", "A");
    }
    if (iii == 2) {
      mueList.set("aggregation: drop scheme", "point-wise");
      mueList.set("aggregation: strength-of-connection: measure",
                  "signed ruge-stueben");
      mueList.set("aggregation: strength-of-connection: matrix",
                  "distance laplacian");
    }
    if (iii == 2) {
      mueList.set("aggregation: drop scheme", "point-wise");
    ...
    for (int kkk = 0; kkk < 9; kkk++) {
      mueList.set("aggregation: drop tol", tolVals[kkk]);
      std::cout << MLList << std::endl;
      RCP<Tpetra_MultiVector> lhsVectorcopy =
          rcp(new Tpetra_MultiVector(*lhsVector, Teuchos::Copy));
#ifdef out
      Teuchos::Array<
          Teuchos::ScalarTraits<Tpetra_MultiVector::scalar_type>::magnitudeType>
          norm_lhs(1), norm_rhs(1), orig(1);
      lhsVectorcopy->norm2(norm_lhs());
      rhsMV->norm2(norm_rhs());
      lhsVector->norm2(orig());
      printf("before solve lhs and rhs = %20.13e    %20.13e | %20.13e \n",
             (double)norm_lhs[0], (double)norm_rhs[0], (double)orig[0]);
#endif
#ifdef out
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeMapFile(
          "rowmap.dat", *(Acrs->getRowMap()));
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeMapFile(
          "colmap.dat", *(Acrs->getColMap()));
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeMapFile(
          "map.dat", *(nCoord->getMap()));
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeSparseFile(
          "Amat.dat", Acrs);
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeDenseFile(
          "coords.dat", nCoord);
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeDenseFile("rhs.dat",
                                                                     rhsMV);
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeDenseFile("init.dat",
                                                                     lhsVector);
#endif
      TestMultiLevelPreconditioner(probType, MLList, Acrs, Mcrs, Minv,
                                   exactNodalVals, rhsMV, lhsVectorcopy, nCoord,
                                   TotalErrorResidual, TotalErrorExactSol);
#ifdef out
      Tpetra::MatrixMarket::Writer<Tpetra_CrsMatrix>::writeDenseFile("sol.dat",
                                                                     lhsVector);
      exit(1);
#endif
#ifdef out
      ...
      rhsMV->norm2(norm_rhs());
      lhsVector->norm2(orig());
      printf("after solve lhs and rhs = %20.13e    %20.13e ! %20.13e\n",
             (double)norm_lhs[0], (double)norm_rhs[0], (double)orig[0]);
#endif
    }
  }
  ...
  TimeMonitor::summarize(Comm.ptr(), std::cout, alwaysWriteLocal,
                         writeGlobalStats, writeZeroTimers, Teuchos::Union,
                         filter, ignoreZeroTimers);
...
const Scalar exactSolution(const Scalar &x, const Scalar &y, const Scalar &z) {
  ...
  // Patch test - hex: tri-linear function is in the FE space and should be
  // recovered return 1. + x + y + z + x*y + x*z + y*z + x*y*z;

  // Analytic solution with homogeneous Dirichlet boundary data for [0 1]x[0
  // 1]x[0 1] return sin(M_PI*x)*sin(M_PI*y)*sin(M_PI*z)*exp(x+y+z);
...
void exactSolutionGrad(Scalar gradExact[3], const Scalar &x, const Scalar &y,
  ...
  // To enable derivatives of the gradient (i.e., 2nd derivatives of the exact
  // solution) need 2 levels of fad types
  ...
  for (int cell = 0; cell < numWorksetCells; cell++) {
    for (int pt = 0; pt < numPoints; pt++) {
      ...
      sourceTermValues(cell, pt) =
          sourceTerm<Sacado::Fad::SFad<double, 3>>(x, y, z).val();
    }
  }
}
...
int TestMultiLevelPreconditioner(
    ...
    Teuchos::RCP<Xpetra::Matrix<
        Tpetra_Vector::scalar_type, Tpetra_Vector::local_ordinal_type,
        Tpetra_Vector::global_ordinal_type, Tpetra_Vector::node_type>> &Minv,
    Teuchos::RCP<Tpetra_Vector> &xexact, Teuchos::RCP<Tpetra_MultiVector> &b,
...
//    mueluParams.sublist("user data").set("Minv", Minv);
  ...
  int numItersPerformed = 0;
  {
    ...
    solveWithBelos(converged, numItersPerformed, solverName, tol, maxNumIters,
                   num_steps, uh, A, b, Teuchos::null, M);
  }
  ...
  if (b->getMap()->getComm()->getRank() == 0) {
    std::cout << std::endl
              << "||X - X_exact||_2 / ||X_exact||_2 = "
              << norm_error[0] / norm_x[0] << std::endl;
  }
...
int getDimension(const ShardsCellTopology &cellTopology) {
  ...
  default:
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "Unknown cell topology for basis selction. Please use Hexahedron_8 or "
        "Tetrahedron_4, Quadrilateral_4 or Triangle_3");
  }
}
