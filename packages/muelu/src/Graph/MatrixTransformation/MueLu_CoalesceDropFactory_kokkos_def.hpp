    ...
    if (threshold != zero) {
      if (socUsesMatrix == "A") {
        ...
        if (status) {
          ...
          TruncMinvA = Xpetra::MatrixMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Multiply(*Minv, false, *A, false, TruncMinvA, GetOStream(Runtime0), true, true, std::string("MinvA"), params);
